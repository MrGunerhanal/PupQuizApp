        ...
        if(stScore==2)
        {
			...
			for (var i = 0; i < pubs.length; i++) {
				if (pubs[i].poi === "St. Paul's Cathedral") {
                 ...
                 displayPubs(poiPubs, "St. Paul's Cathedral");
		});
        }
...
// Formatting for iPad - merging the POI Questions and POI Clue screens
$.each([
	"#towerquestions",
	"#bigbenquestions",
	"#btquestions",
	...
	"#palacequestions",
	"#greenwichquestions"
], function (index, pageSelector) {
	
	// pagebeforecreate is triggered and reorganises the layout
	$( document ).delegate(pageSelector, "pagebeforecreate", function() {
		var page = $(pageSelector);
		
		// Create two column grid and remove popup header.
		page.addClass("ui-grid-a");
		page.find("#container").addClass("ui-block-a");
		page.find("[data-role='popup']").addClass("ui-block-b")
			.find("[data-role='header']").remove();
			
		// Remove popup related elements and properties.
		page.find("[data-role='popup']").removeAttr("data-role");
		page.find("[data-rel='popup']").remove();
		page.find(".popup-close").remove();
	});
});
